...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
            ...
            .position(|x| x.chunk_type().to_string() == chunk_type)
        {
            ...
            None => Err(Box::new(NotFound("Chunk Type not found".into()))),
        }
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let chunk_bytes: Vec<u8> = self.chunks.iter().flat_map(|f| f.as_bytes()).collect();
        Png::STANDARD_HEADER
            .iter()
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        if value[0..8] != Png::STANDARD_HEADER {
            return Err(Box::new(InvalidHeader("Invalid Header".into())));
        }
        let mut chunks: Vec<Chunk> = vec![];
        loop {
            if value.len() >= index + 4 {
                let length = u32::from_be_bytes(value[index..index + 4].try_into().expect("Not 4"));
                if value.len() >= index + 4 + 4 + length as usize + 4 {
                    let chunk =
                        Chunk::try_from(&value[index..index + 4 + 4 + length as usize + 4])?;
                    chunks.push(chunk);
                    index = index + 4 + 4 + length as usize + 4;
                } else {
                    break;
                }
            } else {
